...
// Fungsi mengambil data produk dari Firebase Realtime Database
...
export default async function handler(req, res) {
    // Karena HTML lamamu menembak Groq pakai POST, API ini juga pakai POST
    if (req.method !== 'POST') {
        return res.status(405).json({ error: "Method not allowed" });
    }

    // Mengambil query/pesan teks langsung dari body request HTML lama
    const { messages } = req.body;
    
    // Ambil input text terakhir yang dikirim oleh user
    ...
    try {
        ...
        // Loop rotasi key jika salah satu key terkena limit (429)
        ...
        if (!success) {
            // Struktur response disesuaikan dengan format Groq agar dibaca mulus oleh HTML lama
            return res.status(200).json({
                choices: [{ message: { content: "maaf banget kak, server chat lagi penuh antrean. coba lagi sebentar ya!" } }]
            });
        }

        // Return struktur data tiruan Groq (agar dibaca lancar oleh json.choices[0].message.content di HTML)
        return res.status(200).json(groqResponse);

    } catch (error) {
        console.error(error);
        return res.status(200).json({
            choices: [{ message: { content: "waduh, server lagi pusing nih kak. coba tanya lagi ya." } }]
        });
    }
}
